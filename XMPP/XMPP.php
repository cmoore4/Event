...
class XMPPHP_XMPP extends XMPPHP_XMLStream {
	...
	*/
	...
	public function getPresence($jid){
		$id = $this->getID();
		$this->addIdHandler($id, 'presence_handler');
		if ($this->roster->isContact($jid)) 
			$this->send("<presence type='probe' from='{$this->fulljid}' to='$jid'/>");
	}
	...
	protected function roster_iq_handler($xml) {
		...
		foreach($xmlroster->subs as $item) {
			...
			if ($item->name == 'item') {
				...
				$contacts[] = array($jid, $subscription, $groups); //Store for action if no errors happen
			} else {
				$status = "error";
			}
		}
		if ($status == "result") { //No errors, add contacts
			foreach($contacts as $contact) {
				$this->roster->addContact($contact[0], $contact[1], $contact[2]);
				$this->log->log("Added contact -- {$contact[0]}");
			}
		}
 	...
 	protected function roster_get_handler($xml) {
		...
		//print_r($query);
                foreach ($query->subs as $sub) {
                        ...
                        if ($sub->subs) {
                                $groups = array();
                                foreach (array_keys($sub->subs) as $key) {
                                        $groups[] = $sub->subs[$key]->data;
                                }
                                $roster[$jid] = array_merge($roster[$jid], array('groups' => $groups));
                        }
                }
                // send the event so we know to finish processing
